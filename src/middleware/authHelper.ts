...
export const validateApiSecret = (req:any,res:any,next:any) =>{
    ...
    if(server_mode == "PROD"){
      ...
      if(req.headers["api-key"] == "" || req.headers["api-key"] == undefined){
        return res.status(400).json({message:"api-key is not provided in the headers"});
      }
      ...
      bcrypt.compare(header_api_secret_key,api_secret_key!, function(err, result) {
        if(err){
          console.log(err);
          return res.status(500).json({message:"Error authenticating with the provided api-key"});
        }
        if(!result){
          return res.status(401).json({message:"Authenticaton failed, invalid api-key provided"});
        }
        else{
        next(); 
        }
      ...
      return res.status(500).json({message:"There is an issue with server authentication"});
    }
} 